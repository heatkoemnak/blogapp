...
export function JobLevel({ setSelectedJobLevel }) {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOption, setSelectedOption] = useState('');
  const [jobLevels, setJobLevel] = useState([]);
  ...
  const fetchJobLevel = async () => {
    try {
      const response = await axios.get('/api/job/level');
  ...
  const handleSelect = (level) => {
    ...
    setIsOpen(false);
    setSelectedJobLevel(level); // Update external state if function is passed
  };
      ...
      <div className="max-w-md mx-auto space-y-6">
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <input
              type="text"
              placeholder={selectedOption || 'Select Job Level'}
              readOnly
