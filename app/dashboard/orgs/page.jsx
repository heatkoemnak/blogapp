  ...
  const handleDelete = async (companyId) => {
    ...
    try {
      mutate(
        '/api/companies',
        async (company) => {
          return company?.filter((cpn) => cpn.id !== companyId);
        },
        false
      );
        ...
        <table id="orgTable" className="w-full text-sm text-left rtl:text-right text-gray-800 ">
          ...
          <tbody>
            {data?.map((company) => (
              <tr
                key={company.id}
                className="bg-white border-b dark:bg-gray-900 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600"
              >
